...
class RelationalDBModule(Module):
    ...
    ) -> RelationalDBPersonDetectRepository:
        return RelationalDBPersonDetectRepository(session=session)

    @provider
    def provideAsyncSqlalchemyUnitOfWork(
        self, session: AsyncSession, personDetectionRepo: RelationalDBPersonDetectRepository
    ) -> AbstractUnitOfWork:
        return AsyncSQLAlchemyUnitOfWork(session=session, personDetectionRepo=personDetectionRepo)
...
class DetectionModule(Module):
    @provider
    @singleton
    def provideDetectionEngine(self) -> Detection:
        from settings.person_detect import DetectionEngine
        
        return DetectionEngine
    
    @provider
    def provideDetectionUnitOfWork(self, detectionEngine: Detection) -> AbstractDetectionUnitOfWork:
        return DetectionUnitOfWork(detectionEngine=detectionEngine)


class DetectionModuleFactory:
    def createModule(self):
...
injector = Injector([
    ...
    DetectionModuleFactory().createModule()
])
